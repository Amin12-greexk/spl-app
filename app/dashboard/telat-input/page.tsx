...
export default function TelatInputPage() {
  const { data: session, status } = useSession()
  const router = useRouter()
  const [spls, setSpls] = useState<ManualSpl[]>([])
  const [loading, setLoading] = useState(true)
  const [selectedSpl, setSelectedSpl] = useState<ManualSpl | null>(null)
  ...
  const fetchTelatInput = async () => {
    setLoading(true)
    try {
      const response = await fetch("/api/spl/telat-input")
      if (!response.ok) {
        throw new Error("Gagal mengambil data telat input")
      }
  ...
  const openSignature = (spl: ManualSpl) => {
    setSelectedSpl(spl)
    setSignature("")
  }
    ...
    <div className="space-y-6">
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-xl p-4">
        <div className="flex items-start">
          ...
          <div className="text-sm text-blue-800">
            Setelah Anda tanda tangan, SPL akan langsung diteruskan ke atasan untuk persetujuan.
          </div>
        </div>
      </div>
      ...
      {spls.length === 0 ? (
        <div className="text-center py-12 bg-white rounded-xl border border-gray-200">
          <div className="text-gray-600">Tidak ada SPL telat input</div>
          <div className="text-xs text-gray-500 mt-1">
            Jika ada SPL yang dibuat manual, akan tampil di sini untuk ditandatangani.
          </div>
        </div>
      ) : (
